...
def _obtener_clientes_sin_gestion(df_base, df_revision):
    """Identifica clientes nuevos sin gestión"""
    if df_revision.empty:
        return df_base[df_base["Gestion"].str.strip().eq("")].copy()
    
    # Crear claves compuestas
    df_base["clave"] = df_base["Cuenta"] + "|" + df_base["Base"]
    df_revision["clave"] = df_revision["Cuenta"] + "|" + df_revision["Base"]
    
    # Filtrar clientes no existentes en REVICION y sin gestión
    nuevos = df_base[
        ~df_base["clave"].isin(df_revision["clave"]) & 
        df_base["Gestion"].str.strip().eq("")
    ].copy()
    
    return nuevos.drop(columns=["clave"])
...
def procesar_para_revision():
    ...
    try:
        ...
        # 4. Obtener nuevos clientes sin gestión
        df_nuevos = _obtener_clientes_sin_gestion(df_base, df_rev)
